<div>
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="grid auto-rows-min gap-5 md:grid-cols-3">
            <div>
                <h1 class="text-xl font-semibold mb-4">Éditer <span class=" text-blue-700">{{ $user->name }}</span> </h1>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <button class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Save changes</button>
                </form>
            </div>
            ...
            <div>
                ...
                <div class="rounded-xl border bg-white dark:bg-zinc-900 p-4">
                    ...
                    <div class="text-sm text-zinc-500 dark:text-zinc-300">
                        <span class="font-semibold mb-3">Invitations envoyées</span> : {{ $user->invitation_sent_count ?? 0 }} </br>
                        
                        @if($user->invited_at)
                            <span class="font-semibold">Dernier envoi</span> : {{ optional($user->invited_at)->format('d/m/Y H:i') }}
                        @endif
                    </div>
                    <div class="mt-3">
                        <button wire:click="resendInvitation" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                            Resend Invitation
                        </button>
                    </div>
                </div>
