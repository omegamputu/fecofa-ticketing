    ...
    <body>
        <main class="relative min-h-screen bg-black text-white">
                    ...
                    <nav class="hidden md:block">
                        <ul class="flex items-center">
                            ...
                            <li class="ml-10">
                                ...
                                <div class="text-right text-sm text-zinc-300"> {{ __("Suivi & priorités") }} </div>
                                </a>
                            </li>
                            <li class="ml-10">
                                <a href="#features" class="group block">
                                ...
                                <div class="text-right text-sm text-zinc-300"> {{ __("Attribution technicien") }} </div>
                                </a>
                            </li>
                            <li class="ml-10">
                                <a href="#features" class="group block">
                                ...
                                <div class="text-right text-sm text-zinc-300"> {{ __("Rapports & SLA") }} </div>
                                </a>
                            </li>
                        </ul>
                    </nav>
            ...
            <section class="relative z-10 mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-6 pt-36 pb-16 lg:grid-cols-2 lg:px-10 lg:pt-40">
                <div class="w-full">
                    ...
                    <h2 class="text-4xl font-extrabold tracking-tight sm:text-5xl">
                        {{ __("Gérez vos") }} <span class="text-emerald-400"> {{ __("incidents") }} </span><br class="hidden sm:block"> {{ __("et demandes en toute simplicité") }}
                    </h2>

                    <p class="mt-4 max-w-xl text-zinc-300">
                        {{ __("Déclarez vos problèmes (réseau, imprimantes, accès, applications), suivez l’avancement en temps réel,
                ...
                <div class="relative">
                    <div class="mx-auto w-full max-w-xl rounded-2xl bg-white/5 p-6 shadow-xl ring-1 ring-white/10">
                        <div class="mb-3 flex items-center justify-between">
                            <span class="text-sm font-semibold"> {{ __("Aperçu des tickets") }} </span>
                            <span class="rounded-full bg-white/10 px-2 py-0.5 text-xs"> {{ __("en direct") }} </span>
                        </div>

                        <ul class="space-y-2 text-sm">
                            <li class="flex items-center justify-between rounded-lg bg-white/5 p-3">
                                <span class="truncate">{{ __("Problème d’accès Email - Mot de passe") }}</span>
                                <span class="rounded-md bg-yellow-300/90 px-2 py-0.5 text-xs font-semibold text-yellow-900">En cours</span>
                            </li>
                            <li class="flex items-center justify-between rounded-lg bg-white/5 p-3">
                                <span class="truncate"> {{ __("Imprimante – bourrage papier") }} </span>
                                <span class="rounded-md bg-emerald-300/90 px-2 py-0.5 text-xs font-semibold text-emerald-900">Ouvert</span>
                            </li>
                            <li class="flex items-center justify-between rounded-lg bg-white/5 p-3">
                                <span class="truncate"> {{ __("Réseau – aucun accès internet") }} </span>
            ...
            <footer class="relative z-10 mx-auto w-full max-w-7xl px-6 pb-10 lg:px-10">
                <p class="text-sm font-medium"> {{ __("Service Informatique — FECOFA") }} </p>
